...
static inline void pmCaptureIncomingFramebuffer() {
    ...
    if (g_respect_external_framebuffer) {
        glGetIntegerv(GL_DRAW_FRAMEBUFFER_BINDING, &g_external_incoming_framebuffer);
        if (g_external_incoming_framebuffer == 0) {
            // Fallback to generic binding if draw binding query not supported
...
// Conditional bind that preserves external FBO if flag active and target is default
static inline void pmBindDrawFramebuffer(GLuint targetFramebufferObject) {
    if (g_respect_external_framebuffer) {
...
void ProjectM::RenderFrame(uint32_t targetFramebufferObject /*= 0*/)
{
    ...
    // Capture external framebuffer once per frame (early, after preset render).
    pmCaptureIncomingFramebuffer();
    pmBindDrawFramebuffer(static_cast<GLuint>(targetFramebufferObject));
