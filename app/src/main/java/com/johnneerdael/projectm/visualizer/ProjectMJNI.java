...
public class ProjectMJNI {
    ...
    // Add new dump state native
    private static native void nativeDumpState();
    ...
    public static native void nativeReloadPresets(String presetRootPath);
    ...
    public static void onDrawFrame() {
        try {
            nativeOnDrawFrame();
            dumpState(); // Periodic state dump for debugging
        } catch (Exception e) {
    ...
    // New wrapper to dump native state
    public static void dumpState() {
        try {
            nativeDumpState();
        } catch (UnsatisfiedLinkError e) {
            Log.w(TAG, "nativeDumpState unavailable");
        }
    }
}
