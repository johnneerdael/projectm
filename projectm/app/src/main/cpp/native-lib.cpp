...
static bool g_debug_frame_inspect = true;     // add inspection if missing in this duplicate (for parity)
static bool g_debug_overlay_after_upscale = false;
static int  g_consecutive_black_frames = 0;
static int  g_logged_black_warning_level = 0;
static int  g_fbo_rebind_events = 0;         // count how often projectM escapes our FBO
static const int kRebindDisableThreshold = 6;   // quicker fallback if FBO keeps getting unbound
...
Java_com_johnneerdael_projectm_visualizer_ProjectMJNI_nativeOnSurfaceCreated(JNIEnv *env, jclass clazz, jint width, jint height, jstring preset_path) {
    ...
    // Prefer <root>/presets subdir first; fall back to root if empty
    bool result = false;
